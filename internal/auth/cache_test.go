...
func TestActorCache_EvictionSweepsExpiredBeforeRandom(t *testing.T) {
	cache := NewActorCache(time.Millisecond)
	cache.maxEntries = 3
	...
	// All three are expired. Setting a new entry should sweep them
	// rather than doing a random eviction of a valid entry.
	cache.Set("fresh", ActorInfo{ActorID: "4"})

	if _, ok := cache.Get("a"); ok {
		t.Fatal("expected expired entry 'a' to be swept")
	}
	if _, ok := cache.Get("b"); ok {
		t.Fatal("expected expired entry 'b' to be swept")
	}
	if _, ok := cache.Get("c"); ok {
		t.Fatal("expected expired entry 'c' to be swept")
	}
	if _, ok := cache.Get("fresh"); !ok {
		t.Fatal("expected fresh entry to be present")
	}
	if len(cache.entries) != 1 {
		t.Fatalf("cache size = %d, want 1", len(cache.entries))
	}
}
